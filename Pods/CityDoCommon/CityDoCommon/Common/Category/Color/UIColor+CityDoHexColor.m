...
@implementation UIColor (CityDoHexColor)
...
+ (UIColor *)cd_colorWithHexString:(NSString *)hexString
{
    ...
    return [[self class] cd_colorWithHexString:hexString alpha:alpha];
}
...
+ (UIColor *)cd_colorWithHexString:(NSString *)hexString alpha:(CGFloat)alpha
{
    ...
    if (7 != hexString.length && 4 != hexString.length&&3!=hexString.length) {
        NSString *defaultHex    = [NSString stringWithFormat:@"0xff"];
        unsigned defaultInt = [[self class] hexValueToUnsigned:defaultHex];
        
        UIColor *color = [UIColor colorWith8BitRed:defaultInt green:defaultInt blue:defaultInt alpha:1.0];
        return color;
    }
...
+ (NSString *)hexStringTransformFromThreeCharacters:(NSString *)hexString
{
    if(hexString.length == 4)
    {
        ...
        hexString = [NSString stringWithFormat:@"#%@%@%@",
                     [hexString substringWithRange:NSMakeRange(1, 2)],[hexString substringWithRange:NSMakeRange(1, 2)],[hexString substringWithRange:NSMakeRange(1, 2)]];
    }
