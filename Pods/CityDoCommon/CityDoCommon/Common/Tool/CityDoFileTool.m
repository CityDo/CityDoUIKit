...
+ (NSString *)getFileCachePath:(NSString *)fileName
{
    ...
    NSString * cacheDirectory = [self extracted];
...
{
    if (aFolderNameInCahe == nil || aFolderNameInCahe.length == 0)
    {
        return YES;
    }
...
+ (BOOL)moveFile:(NSString *)file toPath:(NSString *)path {
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSError *error = nil;
    [fileManager moveItemAtPath:file toPath:path error:&error];
    if (error)
    {
        return NO;
    }
